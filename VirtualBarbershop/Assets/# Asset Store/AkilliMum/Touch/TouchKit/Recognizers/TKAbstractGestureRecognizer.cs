    ...
    public abstract class TKAbstractGestureRecognizer : IComparable<TKAbstractGestureRecognizer>
    {
        ...
        public TKGestureRecognizerState state
        {
            ...
            set
            {
                ...
                if (_state == TKGestureRecognizerState.Recognized ||
                    _state == TKGestureRecognizerState.RecognizedAndStillRecognizing)
                    fireRecognizedEvent();
        ...
        private bool shouldAttemptToRecognize
        {
            get
            {
                return (enabled && state != TKGestureRecognizerState.FailedOrEnded &&
                ...
                switch (touch.phase)
                {
                    ...
                    case TouchPhase.Moved:
                    {
                        ...
                        if (!_sentTouchesMoved && populateSubsetOfTouchesBeingTracked(touches) &&
                            _subsetOfTouchesBeingTrackedApplicableToCurrentRecognizer.Contains(touch))
                        {
        ...
        {
            return string.Format("[{0}] state: {1}, location: {2}, zIndex: {3}", this.GetType(), state, touchLocation(),
                zIndex);
        }
