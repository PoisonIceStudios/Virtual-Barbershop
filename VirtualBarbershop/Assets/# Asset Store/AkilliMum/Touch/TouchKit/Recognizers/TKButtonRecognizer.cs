...
namespace AkilliMum.Touches
{
    public class TKButtonRecognizer : TKAbstractGestureRecognizer
    {
        ...
        public TKButtonRecognizer(TKRect defaultFrame, float highlightedExpansion) : this(defaultFrame,
            defaultFrame.copyWithExpansion(highlightedExpansion))
        {
        }
        ...
        internal override void touchesMoved(List<TKTouch> touches)
        {
            for (int i = 0; i < touches.Count; i++)
            {
                if (touches[i].phase == TouchPhase.Stationary)
                {
                    ...
                    else if (state == TKGestureRecognizerState.RecognizedAndStillRecognizing && !isTouchInFrame
                    ) // if the touch exits the frame and we were highlighted deselect now
                    {
