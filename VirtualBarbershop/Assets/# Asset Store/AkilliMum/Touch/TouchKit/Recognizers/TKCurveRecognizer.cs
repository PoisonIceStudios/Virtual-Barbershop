...
namespace AkilliMum.Touches
{
    public class TKCurveRecognizer : TKAbstractGestureRecognizer
    {
        ...
        private Vector2
            _deltaTranslation; //direction vector from previous to current location (current location being the last location far enough from the previous one)
        ...
        internal override bool touchesBegan(List<TKTouch> touches)
        {
            ...
            if (state == TKGestureRecognizerState.Possible ||
                ((state == TKGestureRecognizerState.Began ||
                 ...
                 _trackingTouches.Count < maximumNumberOfTouches))
            {
        ...
        internal override void touchesMoved(List<TKTouch> touches)
        {
            ...
            else if (state == TKGestureRecognizerState.RecognizedAndStillRecognizing ||
                     state == TKGestureRecognizerState.Began)
            {
                ...
                if (delta.sqrMagnitude >= 10f)
                {
                    ...
                    else
                    {
                        ...
                        float crossZ =
                            Vector3.Cross(_previousDeltaTranslation, delta)
                                .z; //  / (_previousDeltaTranslation.magnitude * delta.magnitude);
                        if (crossZ > 0f)
                            deltaRotation -= a;
                        else
                            deltaRotation += a;
        ...
        {
            return string.Format("[{0}] state: {1}, trans: {2}, lastTrans: {3}, totalRot: {4}", this.GetType(), state,
                _deltaTranslation, _previousDeltaTranslation, deltaRotation);
        }
