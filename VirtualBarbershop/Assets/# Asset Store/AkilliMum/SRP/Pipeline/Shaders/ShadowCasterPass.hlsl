...
//#if !defined _AKMU_GEOM_EXTRUDE && !defined _AKMU_GEOM_VOXEL && !defined _AKMU_GEOM_WIREFRAME //they uses their own appends
	...
	for (int i = 0; i < 3; ++i)
	{
		globalOutput[i] = (Varyings)0;
		//use input values as default
		globalOutput[i].positionWS = input[i].positionWS;
		globalOutput[i].positionCS = GetShadowPositionHClip(input[i].positionWS, input[i].normalWS);
		globalOutput[i].uv = input[i].texcoord;
	}
//#endif //global start
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		globalOutput[i].positionCS = GetShadowPositionHClip(globalOutput[i].positionWS, input[i].normalWS);
	}
	...
	wireFrameoutput[0] = (Varyings)0;
	wireFrameoutput[0].positionWS = input[0].positionWS;
	wireFrameoutput[0].positionCS = GetShadowPositionHClip(wireFrameoutput[0].positionWS, normalWSAvg);
	//
	wireFrameoutput[1] = (Varyings)0;
	wireFrameoutput[1].positionWS = input[1].positionWS;
	wireFrameoutput[1].positionCS = GetShadowPositionHClip(wireFrameoutput[1].positionWS, normalWSAvg);
	// pass 2 to 0
	wireFrameoutput[2] = (Varyings)0;
	wireFrameoutput[2].positionWS = input[0].positionWS - dirWireframe[2] * _WireframeShadowSize;
	wireFrameoutput[2].positionCS = GetShadowPositionHClip(wireFrameoutput[2].positionWS, normalWSAvg);
	
	outputStream.Append(wireFrameoutput[0]);
	outputStream.Append(wireFrameoutput[1]);
	outputStream.Append(wireFrameoutput[2]);

	// pass 2 to 1
	wireFrameoutput[2] = (Varyings)0;
	wireFrameoutput[2].positionWS = input[1].positionWS + dirWireframe[1] * _WireframeShadowSize;
	wireFrameoutput[2].positionCS = GetShadowPositionHClip(wireFrameoutput[2].positionWS, normalWSAvg);
	...
	wireFrameoutput[2] = (Varyings)0;
	wireFrameoutput[2].positionWS = input[2].positionWS;
	wireFrameoutput[2].positionCS = GetShadowPositionHClip(wireFrameoutput[2].positionWS, normalWSAvg);
	//pass 0 to 1
	wireFrameoutput[0] = (Varyings)0;
	wireFrameoutput[0].positionWS = input[1].positionWS - dirWireframe[0] * _WireframeShadowSize;
	wireFrameoutput[0].positionCS = GetShadowPositionHClip(wireFrameoutput[0].positionWS, normalWSAvg);
	...
	//pass 0 to 2
	wireFrameoutput[0] = (Varyings)0;
	wireFrameoutput[0].positionWS = input[2].positionWS + dirWireframe[2] * _WireframeShadowSize;
	wireFrameoutput[0].positionCS = GetShadowPositionHClip(wireFrameoutput[0].positionWS, normalWSAvg);
	...
	// pass 1 to 0
	wireFrameoutput[1] = (Varyings)0;
	wireFrameoutput[1].positionWS = input[0].positionWS + dirWireframe[0] * _WireframeShadowSize;
	wireFrameoutput[1].positionCS = GetShadowPositionHClip(wireFrameoutput[1].positionWS, normalWSAvg);
	...
	// pass 1 to 2
	wireFrameoutput[1] = (Varyings)0;
	wireFrameoutput[1].positionWS = input[2].positionWS + dirWireframe[1] * _WireframeShadowSize;
	wireFrameoutput[1].positionCS = GetShadowPositionHClip(wireFrameoutput[1].positionWS, normalWSAvg);
	...
	for (int i = 0; i < 3; ++i)
	{
		wireFrameoutput[i] = (Varyings)0;
		//use input values as default
		wireFrameoutput[i].positionWS = input[i].positionWS;
		wireFrameoutput[i].positionCS = GetShadowPositionHClip(input[i].positionWS, input[i].normalWS);
		wireFrameoutput[i].uv = input[i].texcoord;
	}

	[unroll(3)]
	for (int i = 0; i < 3; ++i)
	{
		outputStream.Append(wireFrameoutput[i]);
	}
