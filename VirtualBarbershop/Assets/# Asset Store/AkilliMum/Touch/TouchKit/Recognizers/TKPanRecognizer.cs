...
namespace AkilliMum.Touches
{
    public class TKPanRecognizer : TKAbstractGestureRecognizer
    {
        ...
        internal override void touchesMoved(List<TKTouch> touches)
        {
            ...
            if (_trackingTouches.Count >= minimumNumberOfTouches && _trackingTouches.Count <= maximumNumberOfTouches)
            {
                ...
                if (state == TKGestureRecognizerState.Began)
                {
                    ...
                    if (System.Math.Abs(totalDeltaMovementInCm) >= _minDistanceToPanCm)
                    {
                        state = TKGestureRecognizerState.RecognizedAndStillRecognizing;
                    }
                }
        ...
        {
            return string.Format("[{0}] state: {1}, location: {2}, deltaTranslation: {3}", this.GetType(), state,
                touchLocation(), deltaTranslation);
        }
