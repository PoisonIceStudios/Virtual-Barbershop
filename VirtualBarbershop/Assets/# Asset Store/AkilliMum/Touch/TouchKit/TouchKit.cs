...
namespace AkilliMum.Touches
{
    ...
    {
        [HideInInspector] public bool simulateTouches = true;
        [HideInInspector] public bool simulateMultitouch = true;
        [HideInInspector] public bool drawTouches = false;
        [HideInInspector] public bool drawDebugBoundaryFrames = false;
        ...
        private Vector2
            _designTimeResolution = new Vector2(320, 180); // 16:9 is a decent starting point for aspect ratio
        ...
        {
            get { return _designTimeResolution; }
            set
            {
        ...
        private bool
            _shouldCheckForLostTouches = false; // used internally to ensure we dont check for lost touches too often
        ...
        public static TouchKit instance
        {
            get
            {
                if (!_instance)
                {
                    ...
                    _instance = FindFirstObjectByType<TouchKit>();
        ...
        {
            _instance.runtimeScaleModifier = new Vector2(Screen.width / _instance.designTimeResolution.x,
                Screen.height / _instance.designTimeResolution.y);
            _instance.runtimeDistanceModifier =
                (_instance.runtimeScaleModifier.x + _instance.runtimeScaleModifier.y) / 2f;
        ...
        public TKRect? getRect()
        {
            return null;
            //var rect = GetComponentInChildren<TouchAreaMarker>()?.GetComponent<RectTransform>();
            //if (rect == null)
            //    return null;
            //var boundary = Extensions.RectTransformToScreenSpace(rect);
            //if (autoScaleRectsAndDistances)
            //{
            //    boundary.x /= runtimeScaleModifier.x;
            //    boundary.y /= runtimeScaleModifier.y;
            //    boundary.width /= runtimeScaleModifier.x;
            //    boundary.height /= runtimeScaleModifier.y;
            //}

            //var tk = new TKRect(boundary.x, boundary.y,boundary.width,boundary.height);
            //return tk;
        }
