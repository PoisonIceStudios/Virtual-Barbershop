...
half3 layer(half2 UV, half T) {
	...
	half w = UV.y * 10;
	half x = (n - 0.5) * 0.8;
	x += (0.4 - abs(x)) * sin(3 * w) * pow(abs(sin(w)), 6) * 0.45;
...
float3 CreateRotation(float4 enviromentRotation, float3 toRotate)
{
    float3 output = toRotate;

    if (enviromentRotation.x != 0)
        Unity_RotateAboutAxis_Degrees_float(output, float3(1, 0, 0), -enviromentRotation.x, output);
    if (enviromentRotation.y != 0)
        Unity_RotateAboutAxis_Degrees_float(output, float3(0, 1, 0), -enviromentRotation.y, output);
    if (enviromentRotation.z != 0)
        Unity_RotateAboutAxis_Degrees_float(output, float3(0, 0, 1), -enviromentRotation.z, output);
