...
namespace AkilliMum.Touches
{
    ...
    public class TKLongPressRecognizer : TKAbstractGestureRecognizer
    {
        ...
        public event Action<TKLongPressRecognizer>
            gestureCompleteEvent; // fired when after a successful long press the finger is lifted
        ...
        internal override bool touchesBegan(List<TKTouch> touches)
        {
            if (!_waiting && state == TKGestureRecognizerState.Possible &&
                (requiredTouchesCount == -1 || touches.Count == requiredTouchesCount))
            {
        ...
        internal override void touchesMoved(List<TKTouch> touches)
        {
                ...
                state == TKGestureRecognizerState.RecognizedAndStillRecognizing)
            {
                ...
                var moveDistance = Vector2.Distance(touches[0].position, _beginLocation) /
                                   TouchKit.instance.ScreenPixelsPerCm;
