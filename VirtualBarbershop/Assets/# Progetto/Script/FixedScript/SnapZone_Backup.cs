...
namespace BNG {
    public class SnapZone_Backup : MonoBehaviour {
        ...
        private HashSet<string> allowNamesSet;
        private HashSet<string> excludeNamesSet;
        ...
        {
            allowNamesSet   = OnlyAllowNames   != null ? new HashSet<string>(OnlyAllowNames)   : new HashSet<string>();
            excludeNamesSet = ExcludeTransformNames != null ? new HashSet<string>(ExcludeTransformNames) : new HashSet<string>();
        }
        ...
        Grabbable getClosestGrabbable() {
            ...
            foreach (var g in gZone.NearbyGrabbables) {
                ...
                if(dist < lastDistance) {
                    ...
                    if (allowNamesSet != null && allowNamesSet.Count > 0) {
                        string transformName = g.Value.transform.name;
                        bool matchFound = false;
                        foreach (string name in allowNamesSet) {
                    ...
                    if (excludeNamesSet != null && excludeNamesSet.Count > 0) {
                        string transformName = g.Value.transform.name;
                        bool excluded = false;
                        foreach (string name in excludeNamesSet) {
                            if (transformName.Contains(name)) {
                                excluded = true;
                                break;
                            }
