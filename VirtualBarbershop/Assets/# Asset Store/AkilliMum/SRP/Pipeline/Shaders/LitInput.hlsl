...
#include "../../Pipeline/Core/ShaderLibrary/DebugMipmapStreamingMacros.hlsl"
...
//global sampler to reduce samplers, already included in globalsaplers.hlsl
...
float _CustomBrightnessIntensity;
...
float _AdditionalLightsColorDistance;
#endif
...
#if defined _AKMU_CARPAINT_WEATHER
...
//float _Wetness;
//half _Rain;
half _AKMU_CARPAINT_RAINY;
uint _RainUV;
float _RainReflectionIntensity;
float _RainStrength;
float _RainSpeed;
float _RainZoom;
float _RainDropStrength;
float _RainBigRivulets;
float _RainSmallRivulets;
...
TEXTURE2D(_TempOpaqueTexture);       SAMPLER(sampler_TempOpaqueTexture);
#endif

#if defined(_AKMU_STYLIZED) && defined(_AKMU_STYLIZED_BRUSH)
...
#if defined(_DETAIL)
    half detailMask = SAMPLE_TEXTURE2D(_DetailMask, sampler_DetailMask, FindUV(_BaseMapUV, uv_0, uv_1, uv_2, uv_3)).a;
    float2 detailUv = FindUV(_BaseMapUV, uv_0, uv_1, uv_2, uv_3) * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
