...
#include "EyeIndex.hlsl"
    ...
    #if (defined(_NORMALMAP) || defined(_AKMU_MIRROR_REFRACTION) || (defined(_PARALLAXMAP) && !defined(REQUIRES_TANGENT_SPACE_VIEW_DIR_INTERPOLATOR))) || defined(_DETAIL)
...
#if defined(_NORMALMAP) || defined(_AKMU_MIRROR_REFRACTION)
...
void InitializeInputData(Varyings input, half3 normalTS, out InputData inputData)
{
    ...
    #if defined(_NORMALMAP) || defined(_DETAIL) || defined(_AKMU_MIRROR_REFRACTION)
...
Varyings LitPassVertex(Attributes input)
{
...
#ifdef _NORMALMAP || _AKMU_MIRROR_REFRACTION
    ...
    output.distance = distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, input.positionOS).xyz);
...
half4 LitPassFragment(Varyings input) : SV_Target
{
    ...
    if (_ClipUV < 99) {
        ...
        //screenUV = input.uv;
        //todo: is it reversed?
        screenUV = 1-input.uv;// +_ClipPercentage / 1000.;// *(_ClipPercentage / 100.) + 0.5;
    }

    //eyeindex.hlsl
    Index_float(screenUV, _IsMultiPass, input.eyeIndex, screenUV);
    ...
    //if(_EnableRefraction > 0){
        ...
        #if UNITY_VERSION >= 201900
        nor = SampleNormal(input.uv, TEXTURE2D_ARGS(_RefractionTex, sampler_RefractionTex), 1);
        #else
        nor = SampleNormal(input.uv, TEXTURE2D_PARAM(_RefractionTex, sampler_RefractionTex), 1);
        #endif
    ...
    {*/
        ...
        noiseOffset *= (_WaveDistortion / input.distance);
        
        reflection = GetReflectionTexture(input.eyeIndex, screenUV + noiseOffset);
    //}
    ...
    //if(_EnableRefraction > 0){
        ...
        #if UNITY_VERSION >= 201900
        half3 bump = SampleNormal(input.uv/_ReflectionRefraction, TEXTURE2D_ARGS(_RefractionTex, sampler_RefractionTex), 1);
        #else
        half3 bump = SampleNormal(input.uv/_ReflectionRefraction, TEXTURE2D_PARAM(_RefractionTex, sampler_RefractionTex), 1);
        #endif
    ...
    {*/
        ...
        if(_ReflectionIntensity > 0){
            ...
            input1 = input.uv + noiseOffset*mask.a;//will be used to animate real texture
    ...
    else if(_EnableSimpleDepth>0)
    {
        ...
        depth = clamp(pow(max(0.0, depth), _SimpleDepthCutoff * input.distance), 0., 1.);
    ...
    //_ReflectionIntensity *= (surfaceData.albedo.x * 0.3 + surfaceData.albedo.y * 0.59 + surfaceData.albedo.z * 0.11);
    ...
    if(_Surface != 1){ //manipulate if surface is not transparent
        /*if(_EnableMask>0){*/
            ...
            /*if(mask.a < 0.2 && mask.a > 0)
                surfaceData.emission =  _ReflectionIntensity * mask.a * dot(input.viewDirWS, input.normalWS);
            else*/
                surfaceData.emission = reflection.rgb * _ReflectionIntensity * pow(max(0.0, mask.a), _MaskEdgeDarkness);
        /*}
        else{*/
        #else
            surfaceData.albedo   = (1-_ReflectionIntensity) * surfaceData.albedo;
            surfaceData.emission = _ReflectionIntensity     * reflection.rgb;
        //}
        #endif
    }
    ...
    if(_Surface == 1){ //transparent mix of reflection for glass like objects
        ...
        if(_WorkType == 30.) //My transparency
        {
            _ReflectionIntensity *= reflection.a;
            
            surfaceData.alpha = _ReflectionIntensity * mask.a;
            ...
            surfaceData.emission = reflection.rgb * mask.a;  //0;
        }
        else{
            surfaceData.albedo = surfaceData.albedo * surfaceData.alpha;
            surfaceData.emission = (reflection.rgb * _ReflectionIntensity) * (1 - surfaceData.alpha) * mask.a;

            //color = color * surfaceData.alpha +
                //half4(reflection.rgb,_ReflectionIntensity) * (1 - surfaceData.alpha);

            surfaceData.alpha = max(_ReflectionIntensity * mask.a, surfaceData.alpha);
        }
    }

    //_ReflectionIntensity = _ReflectionIntensity * 0; // dot(surfaceData.emission, float3(0.3, 0.59, 0.11));
    //surfaceData.emission = (surfaceData.emission.x * 0.3 + surfaceData.emission.y * 0.59 + surfaceData.emission.z * 0.11) / 3;
