...
namespace AkilliMum
{
    ...
    public sealed class MinMaxSlider : PropertyAttribute
    {
        ...
        public MinMaxSlider(float minLimit, float maxLimit)
        {
    ...
    internal sealed class MinMaxSliderDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                throw new ArgumentException("MinMaxSlider underlying value type must be vector2");
            }

            var minmaxAttribute = (MinMaxSlider)base.attribute;
            minLimit = minmaxAttribute.m_MinmaxLimit.x;
            maxLimit = minmaxAttribute.m_MinmaxLimit.y;

            if (minLimit >= maxLimit)
            {
                throw new ArgumentOutOfRangeException("MinMaxSlider min limit can not greater or equal to max limit");
            }
            ...
            if (e.type == EventType.MouseDrag && e.button == 0)
            {
                if (maxValue >= maxLimit)
                {
                    ...
                    if (e.mousePosition.x >= mouseXPosAtRached)
                    {
                        ...
                        float minValueIncreaseAmount = (e.mousePosition.x - mouseXPosAtRached) / sliderWidth * totalRange;
                ...
                else if (minValue <= minLimit)
                {
                    ...
                    if (e.mousePosition.x < mouseXPosAtRached)
                    {
                        // Shrink to the left
                        float totalRange = maxLimit - minLimit;
                        float maxValueDecreaseAmount = (mouseXPosAtRached - e.mousePosition.x) / sliderWidth * totalRange;
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                minmaxAttribute.m_MinmaxValue.x = (float)System.Math.Round(minValue, 1);
                minmaxAttribute.m_MinmaxValue.y = (float)System.Math.Round(maxValue, 1);
