...
#include "../../Pipeline/ShaderLibrary/UnityGBuffer.hlsl"
	...
	#include "../../Pipeline/ShaderLibrary/DeclareOpaqueTexture.hlsl"
...
#endif
#if defined (_AKMU_STYLIZED)
#include "../../Stylized/Shader/StylizedOperations.hlsl"
#endif
...
struct Attributes
{
	...
	float2 staticLightmapUV   : TEXCOORD1; //it is texcoord_1 actually
	float2 dynamicLightmapUV  : TEXCOORD2; //it is texcoord_2 actually
...
#else
	float2  dynamicLightmapUV       : TEXCOORD8; // for uv2 usage
#endif
...
//#endif
	...
	float3 objectPosWSRotated       : TEXCOORD20;
#endif
...
#if defined (_AKMU_LCRS)
	inputData.objectPosWSRotated = input.objectPosWSRotated;
#endif
...
//#ifndef _AKMU_CARPAINT_GEOMETRY
    ...
    output.uv_1 = TRANSFORM_TEX(input.staticLightmapUV * _UV1TileOffset.xy + _UV1TileOffset.zw, _BaseMap);
//#endif
	//output.uv_2 = TRANSFORM_TEX(input.texcoord_2 * _UV2TileOffset.xy + _UV2TileOffset.zw, _BaseMap);
    output.uv_2 = TRANSFORM_TEX(input.dynamicLightmapUV * _UV2TileOffset.xy + _UV2TileOffset.zw, _BaseMap);
    output.uv_3 = TRANSFORM_TEX(input.texcoord_3 * _UV3TileOffset.xy + _UV3TileOffset.zw, _BaseMap);
	
#if defined (_AKMU_CARPAINT) || defined (_AKMU_CARPAINT_GEOMETRY)
	carPaintVertexPosition(input.positionOS, input.normalOS, output.uv, output.uv_1, output.uv_2, output.uv_3);
#endif
...
#if defined (_AKMU_LCRS)
	output.objectPosWSRotated = output.positionWS;
		...
		output.objectPosWSRotated = CreateRotation(_EnviRotation, output.objectPosWSRotated);
		output.objectPosWSRotated += ObjectPosition;
	#endif
#endif
...
#if defined(_AKMU_USE_MIRROR_DEPTH) || defined(_AKMU_DECALTEMP) || defined(_AKMU_DECAL)
	decalVertexProcess(vertexInput.positionVS, output.viewRayOS, output.cameraPosOSAndFogFactor, 0);
#endif
...
FragmentOutput LitGBufferPassFragment(Varyings input)
{
...
#if defined (_AKMU_STYLIZED)
	calculateStylizedColor(input.positionWS, input.normalWS, input.normalOS);
#endif
	...
	half wireframe;
		...
		InitializeStandardLitSurfaceData(input.uv, input.uv_1, input.uv_2, input.uv_3, input.barycentricCoord, input.positionWS, surfaceData, wireframe);
	#else
	...
	#if defined (_AKMU_CARPAINT_GEOMETRY)
		float fresnel = carPaintEffects(input.uv, float2(0,0), input.uv_2, input.uv_3, input.positionWS, input.positionOS,
			input.normalWS, input.normalOS, surfaceData, flakeNormal);
	#else
		float fresnel = carPaintEffects(input.uv, input.uv_1, input.uv_2, input.uv_3, input.positionWS, input.positionOS,
			input.normalWS, input.normalOS, surfaceData, flakeNormal);
	#endif
#endif
...
#if defined (_AKMU_CARPAINT) || defined (_AKMU_CARPAINT_GEOMETRY)
	carPaintApplyColors(surfaceData, fresnel, flakeNormal, input.positionWS, wireframe, input.normalOS);
#else
    surfaceData.albedo *= _Brightness; //car paint uses _Brightness in its own func!
#endif
	
#if defined (_AKMU_CARPAINT) || defined (_AKMU_CARPAINT_GEOMETRY)
	carPaintApplyTexturesDeferred(surfaceData.albedo, surfaceData.metallic, surfaceData.smoothness, input.uv, input.uv_1, input.uv_2, input.uv_3);
#endif
...
//todo: do we need to apply decal,livery etc here?

    Light mainLight = GetMainLight(inputData.shadowCoord, inputData.positionWS, inputData.shadowMask);
    MixRealtimeAndBakedGI(mainLight, inputData.normalWS, inputData.bakedGI, inputData.shadowMask);
#if defined (_AKMU_LCRS)
	half3 color = GlobalIllumination(brdfData, inputData.bakedGI, surfaceData.occlusion, inputData.objectPosWSRotated, inputData.normalWS, inputData.viewDirectionWS);
    ...
    //half3 color = GlobalIlluminationBackWardsCompatiblity(brdfData, inputData.bakedGI, surfaceData.occlusion, inputData.positionWS, inputData.normalWS, inputData.viewDirectionWS);
...
#if defined (_AKMU_MIRROR) && defined (_AKMU_MIRROR_FULL)
	surfaceData.emission = 0; //remove
...
// color.rgb = 0;//sceneRawDepth;
// surfaceData.alpha = 1;
// #endif

#if defined(_AKMU_USE_MIRROR_DEPTH) || defined(_AKMU_DECALTEMP) || defined(_AKMU_DECAL)
	color.rgb = decalLightPrepare(color.rgb, inputData.normalizedScreenSpaceUV);
#endif 
...
#ifdef _AKMU_CARPAINT_GEOMETRY
[maxvertexcount(20)]
void LitGBufferPassGeometry(triangle Varyings input[3], inout TriangleStream<Varyings> outputStream)
{
//global start !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//#if !defined _AKMU_GEOM_EXTRUDE && !defined _AKMU_GEOM_VOXEL && !defined _AKMU_GEOM_WIREFRAME //they uses their own appends
	Varyings globalOutput[3];
	[unroll(3)]
	for (int i = 0; i < 3; ++i)
	{
		globalOutput[i] = input[i]; //use existing parameters
	}
//#endif 
//global start
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		globalOutput[i].positionWS += _AnimSize * ((sin(_Time.y * _AnimSpeed) * PI * 2) + 1) * _AnimDirection;
		//calculate new camera space pos according to new position
		globalOutput[i].positionCS = TransformWorldToHClip(globalOutput[i].positionWS);
	}
	...
	V[0] = globalOutput[0].positionWS;
	V[1] = globalOutput[1].positionWS;
	V[2] = globalOutput[2].positionWS;
	CG = (V[0] + V[1] + V[2]) / 3.;
	...
	{
		//set new pos
		globalOutput[i].positionWS = CG + _ShrinkAmount * (V[i] - CG);
		//set new coord
		globalOutput[i].positionCS = TransformWorldToHClip(globalOutput[i].positionWS);
	}
	...
	float2 uvAvgLowPoly = (globalOutput[0].uv + globalOutput[1].uv + globalOutput[2].uv) / 3;
	float3 normalWSAvgLowPoly = (globalOutput[0].normalWS + globalOutput[1].normalWS + globalOutput[2].normalWS) / 3;
	...
	for (int i = 0; i < 3; ++i)
	{
		if (_SingleColor > 0.5)
		{
			globalOutput[i].uv = uvAvgLowPoly; //open if you wanna distort the UV (texture) too
		}
		globalOutput[i].uv = uvAvgLowPoly;
		globalOutput[i].normalWS = normalWSAvgLowPoly;
	}
#endif //low poly
	...
	InitializeDistanceToEdge(
		globalOutput[0].positionWS, globalOutput[1].positionWS, globalOutput[2].positionWS,
		globalOutput[0].barycentricCoord, globalOutput[1].barycentricCoord, globalOutput[2].barycentricCoord);
...
//global end!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#if !defined _AKMU_GEOM_EXTRUDE && !defined _AKMU_GEOM_VOXEL //they uses their own appends
	//set new output and restart
	[unroll(3)]
	for (int i = 0; i < 3; ++i)
	{
		outputStream.Append(globalOutput[i]);
	}
